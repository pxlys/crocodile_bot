...
# ── Message builders (all use HTML — much more reliable than Markdown) ──
...
def format_scheduled_alert():
    ...
    return (
        ...
        f"  📞 <b>088-251566</b>\n\n"
        f"<i>Use /check_location to check if you are near a danger zone.</i>"
    )
...
def build_zones_text():
    ...
    text += "<i>Use /check_location to check your distance.</i>"
    return text
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/check_location — Check if you are near a danger zone\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # IMPORTANT: use query.message — not update.message (which is None for callbacks)
...
def _send_general_alert():
    if _app_ref:
        asyncio.run_coroutine_threadsafe(
            _app_ref.bot.send_message(chat_id=GROUP_CHAT_ID, text=format_scheduled_alert(), parse_mode="HTML"),
            _app_ref._loop,
        )
...
def _send_fisher_alert():
    if _app_ref:
        asyncio.run_coroutine_threadsafe(
            _app_ref.bot.send_message(chat_id=GROUP_CHAT_ID, text=format_fisher_alert(), parse_mode="HTML"),
            _app_ref._loop,
        )
